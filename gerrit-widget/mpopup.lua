---------------------------------------------------------------------------
--  This is a "fork" of the popup widget in awesome WM (its description below).
--  The fork was created to add support for scrolling
--
...
local function set_position(self)
    ...
    if pf then
        --print("set_position: Called placement function")
...
local function apply_size(self, width, height, set_pos)
    ...
    local max_height = math.floor(mouse.screen.geometry.height * 0.8)
    ...
    if self._private.maximum_height then
		-- don't allow max height to be more than the 80% of the screen
		max_height = math.min(self._private.maximum_height, max_height)
    ...
    --print("Setting next size to be netxt_width", width)
...
-- Layout this widget, if the widget doesn't fit the given dimensions,
-- draw only the part of it (according to how much was scrolled)
function main_widget:layout(context, width, height)
	local offset = self._wb._private.start_offset or 0
	local current_screen = mouse.screen
	local screen_max_height = math.floor(current_screen.geometry.height * 0.8)
	local widget_max_height = math.min(screen_max_height, self._wb._private.maximum_height or 9999)

	if not self._private.widget then
		return 0, 0
	end

    --print("Layout: object", self ,"received width", width, "and height", hight)
    --print("Layout: calling fit for", self._private.widget)
	local w, h = wibox.widget.base.fit_widget(
		self,
		context,
		self._private.widget,
		self._wb._private.maximum_width or 9999,
		9999 -- don't restrict widget in height
	)
	--local w, h = 359, 1344
    
    --print("Layout: Applying w", w, "and h", h)

	-- Setup wibox size. The function would take care of checking maximum_height
	-- value when setting wibox dimensions
	timer.delayed_call(function()
		apply_size(self._wb, w, h, true)
	end)

	-- No need to hassle with scrolling if widget is smaller than
	-- maximum height requested
	if h <= widget_max_height then
		return {
			wibox.widget.base.place_widget_at(self._private.widget, 0, 0, width, height)
		}
	end

	-- If we scrolled to a point that the end of the widget is visible,
	-- don't scroll any more
	if self._wb._private.start_offset + widget_max_height >= h then
		offset = h - widget_max_height
		-- update the wibox offset to the maximum possible scroll
		 self._wb._private.start_offset = offset
	end

	return { 
		wibox.widget.base.place_widget_at(self._private.widget, 0, -offset, width, widget_max_height + offset) 
	}
end
...
function popup:_apply_size_now(skip_set)
    ...
    --print("_apply_size: calling fit for", self:get_widget())
	...
	local w, h = wibox.widget.base.fit_widget(
		...
		9999 -- don't restrict widget in height, since this might affect the width returned as well
	)

	-- crop the height to the min between the requested height and widget max
	-- height
	h = math.min(self._private.maximum_height, h)

	-- ask widget of its dimensions
    ...
    --print("_apply_size: Applying w", w, "and height", h)
...
--- Create a new popup build around a passed in widget (which can be passed in
--declarative form
...
local function create_popup(_, args)
    ...
    rawset(w, "_private", {
		...
		widget				 = child_widget, -- This value is overriden by wibox.set_widget(w, ii)
    })

    gtable.crush(w, popup)

    ii:set_widget(child_widget) -- this value is overriden by wibox.set_widget
    ...
    -- Add scroll support with the mouse
    local buttons = {}
    w._private.start_offset = 0

	-- scroll down
	buttons[4] = function()
		w._private.start_offset = w._private.start_offset - 10
